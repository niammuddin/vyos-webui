$(document).ready(function() {
    ...
    function loadDataTable(tabId) {
      if (tabId === 'aspath' && !loadedData[tabId]) {
        initAspath();
    ...
    function initAspath(){
      var asPathTable = $('#as-path-table').DataTable({
        processing: true,
        responsive: true,
        serverSide: false,
        ajax: {
          url: 'api/policy/as_path.php',
          type: 'POST',
          data: {
            action: 'as-path-list'
        }
      ...
      new $.fn.dataTable.FixedHeader(asPathTable);
    }
    ...
    initAspath();
    loadedData['aspath'] = true;
    $(".nav-link").on("shown.bs.tab", function(e) {
      let targetTabId = $(e.target).attr("aria-controls");
      if (targetTabId === 'nav-as-path-list') {
      loadDataTable('aspath');
